"""Integration smoke tests for SynthPop.

Requires R ('Rscript' on PATH) and the CRAN 'synthpop' package. See
katabatic/models/synthpop/README.md for setup. CI installs both for this
job; skipped locally when Rscript isn't available.
"""
...
@pytest.mark.synthpop
def test_synthpop_artifact_pipeline_smoke(tmp_path, tiny_binary_csv):
    ...
    mr = res["model_ref"]
    assert re.match(
        r"^models/synthpop_smoke_train-\d{8}-\d{6}$",
        mr.root_relpath,
    ), mr.root_relpath
    ...
    state_file = SynthPop.ARTIFACT_STATE_FILES[0]
    ...
    reloaded = SynthPop.load_from_ref(store, mr)
