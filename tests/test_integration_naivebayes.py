"""Integration smoke tests for NaiveBayes."""
...
@pytest.mark.naivebayes
def test_naivebayes_artifact_pipeline_smoke(tmp_path, tiny_binary_csv):
    store = LocalArtifactStore(tmp_path / "artifacts")
    pipe = TrainTestSplitPipeline(model=NaiveBayesModel())
    ...
    mr = res["model_ref"]
    assert re.match(
        r"^models/naivebayes_smoke_train-\d{8}-\d{6}$",
        mr.root_relpath,
    ), mr.root_relpath
    ...
    state_file = NaiveBayesModel.ARTIFACT_STATE_FILES[0]
    ...
    reloaded = NaiveBayesModel.load_from_ref(store, mr)
