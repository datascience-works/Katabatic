"""Integration smoke tests for PrivTree."""
...
from katabatic.models.privtree.models import PrivTreeModel
from katabatic.pipeline.train_test_split.pipeline import TrainTestSplitPipeline


@pytest.mark.integration
@pytest.mark.privtree
def test_privtree_artifact_pipeline_smoke(tmp_path, tiny_binary_csv):
    store = LocalArtifactStore(tmp_path / "artifacts")
    pipe = TrainTestSplitPipeline(model=PrivTreeModel())
    ...
    mr = res["model_ref"]
    assert re.match(
        r"^models/privtree_smoke_train-\d{8}-\d{6}$",
        mr.root_relpath,
    ), mr.root_relpath
    ...
    state_file = PrivTreeModel.ARTIFACT_STATE_FILES[0]
    ...
    reloaded = PrivTreeModel.load_from_ref(store, mr)
