...
def build_paths(config: RunConfig) -> dict:
    ...
    return {
        ...
        "processed_data": os.path.join(
            benchmarks_dir,
            "processed",
            f"{config.dataset_name}_processed.csv",
        ),
...
def preprocess_and_split(config: RunConfig):
    ...
    Returns (train_df, test_df, target_col, paths).
    """
    ...
    os.makedirs(
        os.path.dirname(paths["processed_data"]),
        exist_ok=True,
    )

    processed_meta_path = paths["processed_data"].replace(
        ".csv",
        "_meta.json",
    )
    ...
    if os.path.exists(paths["processed_data"]) and _preprocess_cache_valid(
        ...
        print(
            "Processed data already exists and parameters match, "
        ...
        _write_meta(
            processed_meta_path,
            {
                "target_col": config.target_col_raw,
            },
        )
    ...
    if (
        ...
        print(
            ...
            f"max_train_rows={config.max_train_rows:,}."
        )
        ...
        print(
            ...
            f"{train_df[target_col].value_counts().to_dict()}"
        )
...
def evaluate(
    ...
    """Run the evaluation pipeline and print the final summary.
