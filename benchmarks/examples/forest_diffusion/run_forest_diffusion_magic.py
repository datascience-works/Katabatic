...
def prepare_forest_data(
    ...
    """Encode the target and prepare numerical training data."""
    required_columns = [
        *NUMERICAL_COLUMNS,
        target_col,
    ]
...
def decode_forest_output(
    ...
    """Restore target labels and numerical values."""
    ...
    # The uploaded Magic dataset uses integer values.
    for column in NUMERICAL_COLUMNS:
        values = pd.to_numeric(
            synthetic_df[column],
            errors="raise",
...
def main() -> None:
    ...
    print("\nSTEP 1 — Preprocess and split Magic")
    ...
    # Needed by stability evaluation.
    model.category_maps = category_maps
    ...
    print("\nSTEP 5 — Check target generation")
    ...
    print(
        "Real target classes:",
        sorted(train_df[target_col].astype(str).unique()),
    )

    print(
        "Synthetic target classes:",
        sorted(test_sample[target_col].astype(str).unique()),
    )
