...
def prepare_forest_data(
    ...
    """Encode Nursery features and target."""
    required_columns = [
        *CATEGORICAL_COLUMNS,
        target_col,
    ]
    ...
    # Target is included so features and target
...
def decode_forest_output(
    ...
    """Convert category codes back to Nursery labels."""
...
def main() -> None:
    ...
    print("\nSTEP 1 — Preprocess and split Nursery")
    ...
    # Required so sample() returns original labels.
    ...
    for column in [
        *CATEGORICAL_COLUMNS,
        ...
        matched_categories = real_categories & sampled_categories

        print(
            f"{column}: "
            f"{len(matched_categories)}/"
            f"{len(real_categories)} categories matched"
        )
