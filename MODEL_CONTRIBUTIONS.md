...
3. **CI checks are green**: add the model to the `changes` job's `ALL_MODELS` list and its `paths-filter` block in [.github/workflows/ci.yml](.github/workflows/ci.yml), so `integration-<model_name>` runs on relevant changes. `model-contract-<model_name>` needs no separate YAML change — its matrix is derived from `ModelRegistry.get_supported_models()` at CI run-time, so it appears automatically once step 5 below is done. Both jobs need to pass.
