...
class Tabddpm(Model):
    ...
    def train(
        ...
        **kwargs,
            ...
            if self._is_classification:
                ...
                if "label" in df_synth.columns:
                    ...
                    vals, counts = np.unique(
                        self._as_numpy(y_train).ravel(), return_counts=True
                    )
                    probs = counts / counts.sum()
                    y_synth = pd.Series(
                        np.random.choice(vals, size=n_rows, p=probs), name=y_col_name
                    )
        ...
        if len(args) < 2:
            raise TypeError("train() missing required positional arguments: X, y")
        X, y = args[0], args[1]
        ...
        if isinstance(X, pd.DataFrame):
            ...
            cat_cols = [
                all_cols[i] if isinstance(i, int) else i for i in (cat_cols or [])
            ]
        ...
        if X_cat is None or X_cat.shape[1] == 0:
            ...
            self._K = np.array(
                [int(np.max(X_cat[:, j]) + 1) for j in range(X_cat.shape[1])], dtype=int
            )
        ...
        if _TABDDPM_EXTERNAL:
            d_in = (
                0 if self._K.size == 1 and self._K[0] == 0 else int(self._K.sum())
            ) + self._n_num
        else:
        ...
        rtdl_params = dict(
            d_layers=list(self._cfg["d_layers"]), dropout=float(self._cfg["dropout"])
        )
        ...
        self._diffusion = (
            GaussianMultinomialDiffusion(
                num_classes=self._K
                if not (self._K.size == 1 and self._K[0] == 0)
        ...
        for step in range(steps):
            ...
            if self._is_classification:
                ...
                out = {
                    "y": torch.zeros(xb.shape[0], dtype=torch.long, device=self.device)
                }
        ...
        *,
        ...
        if X is None or y is None:
            ...
            if (
                ...
                and self._y_le is not None
                and not np.issubdtype(y_np.dtype, np.integer)
            ):
    ...
    def sample(
        ...
        *,
        ...
        y_dist = (
            self._class_dist
            if self._class_dist is not None
        ...
        if self._K.size > 0 and not (self._K.size == 1 and self._K[0] == 0):
            ...
            cat_df = pd.DataFrame(cat_block, columns=self._feature_names_cat).astype(
                int
            )
