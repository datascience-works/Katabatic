...
class CTABGANSynthesizer:
    ...
    def fit(self, train_data, categorical, mixed, general, non_categorical, type):
        ...
        for epoch in range(self.epochs):
            ...
            for start in range(0, len(data), self.batch_size):
                ...
                if self.classifier is not None:
                    fake_pred_g, fake_label_g = self.classifier(fake)
                    target_dim = self.classifier.str_end[1] - self.classifier.str_end[0]
                    if target_dim == 1:
                        loss_cg = F.binary_cross_entropy_with_logits(
    ...
    def _gen_layers(self):
        ...
        base = self.num_channels * 4
        layers = [
            ConvTranspose2d(self.random_dim, base, 4, 1, 0),
            ReLU(True),
        ]

        current_size = 4
        current_channels = base

        while current_size < self.Gtransformer.height:
            next_channels = max(current_channels // 2, self.num_channels)
    ...
    def _dis_layers(self):
        ...
        return [
            Conv2d(1, ch, 4, 2, 1),
            LeakyReLU(0.2),
            Conv2d(ch, ch * 2, 4, 2, 1),
            LeakyReLU(0.2),
            torch.nn.Flatten(),
            Linear(ch * 2 * reduced_side * reduced_side, 1),
        ]
...
class CTABGANPlus(Model):
    ...
    def train(
        ...
        **kwargs,
        ...
        # Convert column names to integer indices expected by DataPrep
        all_cols = list(train_df.columns)
        if categorical_cols is not None:
            categorical_indices = [
                all_cols.index(col) for col in categorical_cols if col in all_cols
            ]
        else:
            categorical_indices = []

        # Target column must be label-encoded too (it's always categorical for classification)
