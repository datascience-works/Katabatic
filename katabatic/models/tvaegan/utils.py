...
class DecoderGenerator(torch.nn.Module):
    def __init__(self, in_size: int, hidden_dims: List[int], out_size: int, cat_size: int, num_size: int, dropout: float, device=None):
        super().__init__()
        self.device = device
...
class TVAEGANSynthesizer(BaseEstimator):
    def __init__(
        ...
        self.dropout = dropout
        self.shuffle = shuffle
        self.ot_loss = ot_loss or {"loss": "energy"}
    ...
    def fit(self, df: pd.DataFrame):
        ...
        self.model = TVAEGANNetwork(
            ...
            cat_size=sum(cat_sizes),
            num_size=len(num_sizes),
    ...
    def predict(self, samples: int) -> pd.DataFrame:
