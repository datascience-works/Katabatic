...
class CODI(Model):
    ...
    def _train_step(
        ...
        if self.has_continuous_:
            ...
            # Paper alignment:
            # CoDi represents discrete variables in one-hot categorical space.
            # The continuous diffusion model is therefore conditioned on the
            # one-hot representation rather than integer-encoded category columns.
            ...
            # Paper alignment:
            # Negative conditions are generated by shuffling complete categorical
            # records while preserving the relationships between categorical
            # variables within each record.
            neg_indices = torch.randperm(batch_size, device=x_cat.device)
            x_cat_neg = x_cat[neg_indices]
            x_cat_neg_onehot = self._to_onehot(x_cat_neg)
        
            # Current compatibility implementation:
            # Use the same one-hot categorical representation for both positive
            # and negative conditions so that the condition dimensions match
            # the continuous diffusion network input.
            ...
            loss_con_contrast = torch.relu(
                F.mse_loss(
                    eps_neg, noise_con, reduction='none'
                ).mean()
                - F.mse_loss(
                    eps_pos, noise_con, reduction='none'
                ).mean()
                + 1.0
            )
            ...
            loss_con_total = (
                loss_con_diff
                + self.lambda_con * loss_con_contrast
            )
    ...
    def sample(
        ...
        # Restore the original dataset column order before returning synthetic data.
        # CoDi internally separates continuous and categorical features, which changes
        # their order when the generated data is reconstructed. TSTR estimators require
        # synthetic and real feature columns to appear in the same order.
        # This change only restores schema consistency and does not modify generated values.
        ordered_cols = [
            col
            for col in self.schema_['column_order']
