...
class TabSyn(BaseModel):
    ...
    def train(
        ...
        **kwargs
        ...
        # FIX: force y_synth to follow real y_train label distribution
        real_y_path = os.path.join(data_dir, "y_train.csv")
        real_y = pd.read_csv(real_y_path).iloc[:, 0].astype(int).to_numpy()

        classes, counts = np.unique(real_y, return_counts=True)
        probs = counts / counts.sum()

        y_synth = np.random.choice(classes, size=len(x_synth), p=probs).astype("int64")
