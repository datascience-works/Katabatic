...
class StabilityEvaluation(Evaluation):
    ...
    def evaluate(self) -> dict:
        ...
        for i, seed in enumerate(self.seeds):
            ...
            if synth is None:
                print(f"  [Stability] WARNING: sample() returned None for seed={seed}, skipping run.")
                continue

            fid_result = FidelityEvaluation(
                self.real_data, synth,
        ...
        mean_scores = {dim: round(float(np.mean(vals)), 4) for dim, vals in per_run.items() if vals}
        std_scores  = {dim: round(float(np.std(vals)),  4) for dim, vals in per_run.items() if vals}
    ...
    def _print_summary(self, results):
        ...
        for i, r in enumerate(results['per_run_scores']):
            print(f"{i + 1:<6} {r['seed']:<8} {r['fidelity_score']:<12.4f} {r['diversity_score']:<12.4f}")
        print("-" * 40)
        print(f"{'Mean':<6} {'':<8} {results['mean_scores'].get('fidelity', '-'):<12} {results['mean_scores'].get('diversity', '-'):<12}")
        print(f"{'Std':<6} {'':<8} {results['std_scores'].get('fidelity', '-'):<12} {results['std_scores'].get('diversity', '-'):<12}")
        print(f"\nAverage std across dimensions: {results['avg_std']:.4f}  (target < 0.05)")
        flag = "  [WARNING] High variance — model is unstable." if results['avg_std'] >= 0.05 else "  [OK]"
