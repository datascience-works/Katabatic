...
import utils
from utils import get_dimwise_prob_metrics, save_current_plot, score_real_fake, make_dimwise_probability_plot, \
...
class BaseGAN():
    ...
    def generate_df(self, n=5000, sampling_rate={50: 50}, column_names=None, as_numpy=False):
        """
        Generate synthetic samples for both minority and majority classes using the conditional GAN,
        according to a specified class distribution.
        ...
        sampling_rate : dict, default={50: 50}
            A dictionary specifying the percentage split between minority and majority classes.
            The key represents the percentage of the minority class (label 1),
            and the value represents the percentage of the majority class (label 0).
            Example: {30: 70} will generate 30% minority and 70% majority samples.

        column_names : list of str, optional
            A list of column names for the returned DataFrame.
            Must match the number of columns in the generated data (including the target label).
            If None, default names will be used in the format ['feature_0', ..., 'feature_n', 'target'].

        as_numpy : bool, default=False
            If True, returns the generated data as a NumPy array.
            If False, returns a pandas DataFrame.

        Returns
        -------
        np.ndarray or pandas.DataFrame
            Synthetic dataset with the specified class distribution.
            The last column or the column named 'target' represents the class label (0 or 1).

        Raises
        ------
        ValueError
            - If `sampling_rate` is not a valid dictionary or does not sum to 100%.
            - If `column_names` is provided and does not match the expected number of columns.
            - If the GAN is not conditional (`self.netG.condition` is False).

        Notes
        -----
        - This method is intended for use with conditional GANs that support class-conditioned sampling.
        - The output can be used to augment training datasets with synthetic balanced or imbalanced samples,
        depending on your target class distribution.

        """

        if not self.netG.condition:
            raise ValueError("sample_balanced requires conditional GAN (netG.condition=True)")

        # Parse the sampling_rate dict
        try:
            minority_pct = list(sampling_rate.keys())[0] / 100
            majority_pct = list(sampling_rate.values())[0] / 100
        except Exception:
            raise ValueError("sampling_rate must be a dictionary like {70: 30}")

        if abs(minority_pct + majority_pct - 1.0) > 1e-3:
            raise ValueError("sampling_rate percentages must sum to 100")

        n_minority = int(n * minority_pct)
        n_majority = n - n_minority

        # Sample minority class (y = 1)
        X_minority = self.sample(n=n_minority, y=1, as_numpy=True)
        y_minority = np.ones((n_minority, 1))

        # Sample majority class (y = 0)
        X_majority = self.sample(n=n_majority, y=0, as_numpy=True)
        y_majority = np.zeros((n_majority, 1))

        # Append labels
        X_minority_labeled = np.hstack([X_minority, y_minority])
        X_majority_labeled = np.hstack([X_majority, y_majority])

        # Combine both classes
        X_balanced = np.vstack([X_minority_labeled, X_majority_labeled])

        if as_numpy:
            return X_balanced  # return as raw NumPy array if requested

        # Convert to DataFrame
        n_features = X_balanced.shape[1] - 1
        if column_names is None:
            column_names = [f"feature_{i}" for i in range(n_features)] + ["target"]
        elif len(column_names) != X_balanced.shape[1]:
            raise ValueError("Number of column_names must match number of columns in the data")

        df = pd.DataFrame(X_balanced, columns=column_names)
        return df
