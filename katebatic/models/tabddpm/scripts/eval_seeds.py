...
def eval_seeds(
    ...
    with tempfile.TemporaryDirectory() as dir_:
        ...
        for sample_seed in range(n_datasets):
            ...
            # Check if we need to regenerate synthetic data
            should_regenerate = eval_type != 'real' and n_datasets > 1 and not (parent_dir / "y_train.npy").exists()

            if should_regenerate:
                subprocess.run(['python3.9', f'{pipeline[sampling_method]}', '--config', f'{str(dir_ / "config.toml")}', '--sample'], check=True)
            elif eval_type == 'synthetic':
                # Copy existing synthetic data to temp directory
                print(f"Copying existing synthetic data to temp directory...")
                synthetic_files = [
                ...
                for file in synthetic_files:
                    src = parent_dir / file
                    dst = Path(temp_config["parent_dir"]) / file
                    if src.exists():
