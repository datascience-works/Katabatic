# katebatic/models/tableGAN/models.py
"""
...
# Use the project's base class
from ..base_model import Model

# Import your helpers (make sure ops you used are moved into utils.py)
from .utils import *  # batch_norm, conv2d, deconv2d, linear, lrelu, concat, conv_cond_concat, ...
                      # histogram_summary, scalar_summary, image_summary, merge_summary, SummaryWriter
                      # masking, padding_duplicating, reshape
...
# ---------------------------
# Legacy implementation (unchanged logic, just renamed)
# ---------------------------
class LegacyTableGan(object):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
                 ...
                 label_col=-1, attrib_num=0, is_shadow_gan=False, test_id=''):
        """
    ...
    def build_model(self):
        ...
        # Classifier Loss
        if self.y_dim:
            self.c_loss = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.C_logits, y_normal))
            self.g_loss_c = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.C_logits_, y_normal))

        # Original GAN Loss
        self.g_loss_o = tf.reduce_mean(sigmoid_cross_entropy_with_logits(self.D_logits_, tf.ones_like(self.D_)))

        # Info loss (feature statistics)
    ...
    def train(self, config, experiment):
        ...
        for epoch in xrange(config.epoch):
            ...
            np.random.seed(seed); np.random.shuffle(self.data_X)
            if self.y_dim:
                np.random.seed(seed); np.random.shuffle(self.data_y)
                np.random.seed(seed); np.random.shuffle(self.data_y_normal)
            ...
            for idx in xrange(0, batch_idxs - 1):
                ...
                if self.y_dim:
                    _, summary_str = self.sess.run([d_optim, self.d_sum], feed_dict={
                        self.inputs: batch_images, self.z: batch_z, self.y: batch_labels, self.y_normal: batch_labels_normal
                    })
                    self.writer.add_summary(summary_str, counter)

                    # Update C
                    _, summary_str = self.sess.run([c_optim, self.c_sum], feed_dict={
                        self.inputs: batch_images, self.z: batch_z, self.y: batch_labels, self.y_normal: batch_labels_normal
                    })
                    self.writer.add_summary(summary_str, counter)
                    ...
                    _, summary_str, gmean, gmean_, gvar, gvar_ = self.sess.run(
                        ...
                        feed_dict={ self.z: batch_z, self.y: batch_labels, self.inputs: batch_images,
                                    self.y_normal: batch_labels_normal, self.prev_gmean: gmean,
                                    self.prev_gmean_: gmean_, self.prev_gvar: gvar, self.prev_gvar_: gvar_ })
                    self.writer.add_summary(summary_str, counter)
                    ...
                    errC = self.c_loss.eval({ self.inputs: batch_images, self.z: batch_z,
                                              self.y: batch_labels, self.y_normal: batch_labels_normal })
                    errG = self.g_loss.eval({ self.z: batch_z, self.y: batch_labels, self.y_normal: batch_labels_normal,
                                              self.inputs: batch_images, self.prev_gmean: gmean, self.prev_gmean_: gmean_,
                                              self.prev_gvar: gvar, self.prev_gvar_: gvar_ })
                    ...
                    _, summary_str, gmean, gmean_, gvar, gvar_ = self.sess.run(
                        ...
                        feed_dict={ self.z: batch_z, self.inputs: batch_images,
                                    self.prev_gmean: gmean, self.prev_gmean_: gmean_,
                                    self.prev_gvar: gvar, self.prev_gvar_: gvar_ })
                    self.writer.add_summary(summary_str, counter)
                ...
                if np.mod(counter, 100) == 1:
                    if self.y_dim:
                        samples, d_loss, c_loss, g_loss = self.sess.run(
                            ...
                            feed_dict={
                                self.z: sample_z, self.inputs: sample_inputs, self.y: sample_labels,
                                self.y_normal: sample_labels_normal, self.prev_gmean: gmean,
                                self.prev_gmean_: gmean_, self.prev_gvar: gvar, self.prev_gvar_: gvar_
                            })
    ...
    def load_tabular_data(self, dataset_name, dim, classes=2, test_id='', load_fake_data=False):
        ...
        if os.path.exists(self.train_data_path + ".csv"):
            ...
            print("Error Loading Dataset !!"); exit(1)
...
# ---------------------------
# Standardized wrapper that conforms to katebatic.models.base_model.Model
# ---------------------------
class TableGAN(Model):
    def __init__(self, config, experiment=None):
        ...
        # Legacy implementation instance
        self.impl = LegacyTableGan(
            sess=self.sess,
            input_height=config.input_height,
            input_width=config.input_width,
            batch_size=config.batch_size,
            sample_num=getattr(config, "sample_num", 64),
            output_height=config.output_height,
            output_width=config.output_width,
            y_dim=getattr(config, "y_dim", None),
            z_dim=getattr(config, "z_dim", 100),
            gf_dim=getattr(config, "gf_dim", 64),
            df_dim=getattr(config, "df_dim", 64),
            gfc_dim=getattr(config, "gfc_dim", 1024),
            dfc_dim=getattr(config, "dfc_dim", 1024),
            dataset_name=config.dataset,
            checkpoint_dir=config.checkpoint_dir,
            sample_dir=getattr(config, "sample_dir", None),
            alpha=getattr(config, "alpha", 1.0),
            beta=getattr(config, "beta", 1.0),
            delta_mean=getattr(config, "delta_mean", 0.0),
            delta_var=getattr(config, "delta_var", 0.0),
            label_col=getattr(config, "label_col", -1),
            attrib_num=getattr(config, "attrib_num", 0),
            is_shadow_gan=getattr(config, "is_shadow_gan", False),
            test_id=getattr(config, "test_id", "")
        )

    @classmethod
    def get_required_dependencies(cls) -> list[str]:
        # Keep this aligned with actual imports used
        return ["tensorflow", "numpy", "pandas", "scikit-learn"]

    def train(self, *args, **kwargs) -> "Model":
        """Train and return self (per base Model contract)."""
        self.impl.train(self.config, self.experiment)
        self.is_fitted = True
        return self

    def evaluate(self, data=None, **kwargs) -> float:
        """
        Return a single scalar score (e.g., generator loss on a small probe batch).
        """
        try:
            z_dim = getattr(self.config, "z_dim", 100)
            bsz = min(getattr(self.config, "batch_size", 64), 16)
            z = np.random.uniform(-1, 1, size=(bsz, z_dim)).astype(np.float32)

            feed = { self.impl.z: z }

            if getattr(self.config, "y_dim", None):
                ydim = self.config.y_dim
                y = np.zeros((bsz, ydim), dtype=np.float32); y[:, 0] = 1.0
                feed[self.impl.y] = y

                # need real inputs for d_loss terms used in graph
                X = self.impl.data_X[:bsz]
                X = np.array(X, dtype=np.float32)
                if self.impl.grayscale:
                    X = X[:, :, :, None]
                feed[self.impl.inputs] = X

                if hasattr(self.impl, "y_normal") and self.impl.data_y_normal is not None:
                    feed[self.impl.y_normal] = self.impl.data_y_normal[:bsz]

            g_loss = self.sess.run(self.impl.g_loss, feed_dict=feed)
            return float(np.mean(g_loss))
        except Exception:
            # In case evaluation graph pieces aren't available yet
            return float("nan")

    def sample(self, num_samples=64, y=None, **kwargs):
        """Generate synthetic samples using the legacy sampler graph."""
        z_dim = getattr(self.config, "z_dim", 100)
        z = np.random.uniform(-1, 1, size=(num_samples, z_dim)).astype(np.float32)
        feed = { self.impl.z: z }
        if getattr(self.config, "y_dim", None) and y is not None:
            feed[self.impl.y] = y  # shape must match (num_samples, y_dim)
        samples = self.sess.run(self.impl.sampler, feed_dict=feed)
        return samples
